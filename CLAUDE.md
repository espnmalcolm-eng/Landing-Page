...
0. **Globe moment** — cinematic opener, first section (`#globe-moment`, 100vh desktop + mobile). **Static globe image + transparent WebGL particle overlay** (Three.js r128, CDN, no build step). The globe *surface* is no longer 3D — it's the static `assets/globe.png` (`assets/globe-mobile.png` under 768px) inside a `.globe-crop` box (`position:relative; overflow:hidden`; 620px tall desktop / 420px mobile; image `width:115%` desktop / `195%` mobile, bottom-anchored, with a contrast/saturate/brightness + gold drop-shadow filter). A **transparent WebGL canvas is mounted inside `.globe-crop`** (`alpha:true`, `setClearColor(0x000000, 0)`, sized to the crop, `pointer-events:none`) and draws **only**: 38 named-city **additive glow sprites** (shared radial-gradient CanvasTexture, `0xD4B86A`, `depthTest:false`; primary hubs scale `0.072` / secondary `0.040`) + a seeded (`mulberry32`) additive glow-**Points** cloud (420 desktop / 70 mobile, `size:0.032`) clustered near the metros + arcs: 3 **primary** layered draw-in arcs (NYC→London/Lagos/Atlanta, each 4 additive line layers at opacity ×1/0.65/0.38/0.14 and scale 1/1.006/1.016/1.034 to fake glow/thickness; `setDrawRange` animates them in on section-enter) and ~55 low-opacity **secondary** arcs wrapping the whole globe. **There is NO sphere, atmosphere, lighting, earth texture, lat/lng grid, or sun-flare anymore** — all removed when the surface became a static image. Camera matches the photo's Atlantic-facing, slightly-elevated viewpoint: FOV 30 (mobile 50), `position (0, 0.6, 4.0)` desktop / `(0, 0.6, 3.0)` mobile, `lookAt(0, GY, 0)`; the particle group is `scale 1.6` (mobile 1.15), `y -1.15` (mobile -0.78), fixed `rotation.y = π*0.22` (`BASE_RY`), `rotation.x = -0.28` (`BASE_RX`).
   ...
   - **Scroll:** legacy parallax `translateX` drift (≤30px) + opacity dissolve — but this is **superseded by the dissolve transition** once it fires (the transition owns the canvas/crop styling from then on).
   - **Dissolve transition → hero (`#hero`):** a 1px sentinel at `top:75%` of the section, watched by an IntersectionObserver with `rootMargin:'0px 0px -80% 0px'` (so it only triggers when it rises into the top 20% of the viewport on a downward scroll-through, NOT at load). On fire (once; `hasTransitioned` guard, re-armed by `resetTransition()` when scrolled back to the top): **Phase 1** CSS `transform:scale()` push-in on `.globe-crop` (static image + canvas zoom together) toward the NYC hub (`transform-origin` ~`38% 65%` desktop / `40% 60%` mobile); **Phase 2** (~60% in) particle scatter — each city sprite flies outward to `1.8×` over 800ms with 0–200ms per-sprite delay and fades, the bg Points cloud expands+fades via a single buffer tween, arcs fade over 600ms; **Phase 3** (final 400ms) `.globe-crop` opacity→0; **completion** (`onZoomComplete`, at DUR = 1800ms desktop / 1400ms mobile) calls `window.__heroNetField.boostBrightness(1.3, 1000)` so the hero StarField pulses 30% brighter and eases back over 1s — selling the globe's lights "joining" the field. `prefers-reduced-motion` → plain 0.6s crop crossfade (no zoom/scatter, boost still fires). Mobile scatters primary cities only, gentler zoom, shorter DUR; a marked comment shows where to swap to the crossfade if momentum-scroll jank appears. Render-pause IntersectionObserver is held open while `isTransitioning`; all `setTimeout`/`rAF` refs are tracked and cleared; `renderer.dispose()` + canvas removal fire 3s after completion (skipped if re-armed).
   - **Hero StarField is always-on from page load** (never scroll-gated) so it's already rendering under the globe when the dissolve fires; `NetField.boostBrightness(factor, duration)` multiplies every particle's alpha then eases back to 1. The hero field is exposed as `window.__heroNetField`.
